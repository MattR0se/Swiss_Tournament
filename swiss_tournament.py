...
class tournament():
    ...
    def enter_result(self, table, result, swiss=True):
        ...
        if table in self.results_entered:
            string = '--- WARNING: Result already entered! ---'
            return string
        elif (player1_win + player2_win + draw) > 3:
            string = '--- WARNING: Invalid Result! ---'
            return string
        else:
        ...
        string = 'Result entered: Table {0}, {1} VS. {2}   {3}/{4}/{5}'.format(
                                        table, self.pairings[table][0].name,
                                        self.pairings[table][1].name, 
